class Solution:
    def numUniqueEmails(self, emails: list[str]) -> int:
        """
        Given a list of strings emails where we send one email to each
        ...
        :param list[str] emails: strings emails
        :returns int unique: number of different addresses that actually receive
        mails

        Time complexity: O(n)
        Space complexity: O(n)

        LeetCode: Beats 98.34%
        """
        unique = set()
