class Solution:
    def heightChecker(self, heights: list[int]) -> int:
        """
        A school is trying to take an annual photo of all the students. The
        students are asked to stand in a single file line in non-decreasing
        order by height. Let this ordering be represented by the integer list
        expected where expected[i] is the expected height of the ith student in
        line.

        You are given an integer list heights representing the current order
        that the students are standing in. Each heights[i] is the height of the
        ith student in line (0-indexed).

        Return the number of indices where heights[i] != expected[i].

        :param list[int] heights: current order that the students are standing in
        :returns int count: number of indices where heights[i] != expected[i]

        Time complexity: O(nlogn)
        Space complexity: O(n)

        LeetCode: Beats 96.40%
        """
        count = 0

        for a, b in zip(heights, sorted(heights)):
            if a != b:
                count += 1

        return count
